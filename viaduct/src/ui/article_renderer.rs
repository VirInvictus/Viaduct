...
// Phase 20d: `apply_app_accent` is removed. It painted the article theme's
// accent across the GTK chrome (v1.2.0), which fought the owned Kanagawa
// stylesheet (theme.rs). The chrome is Kanagawa; the article pane uses its
// theme accent directly in `render_themed`.

// Phase 20d: `pick_accent_fg` + `relative_luminance` removed with
// `apply_app_accent` (their only caller). WCAG accent-fg picking was for
// the chrome-accent propagation; the owned Kanagawa sheet fixes its own
// on-accent colour.
