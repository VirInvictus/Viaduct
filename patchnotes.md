...
- **Documented:** the 2026-09-06 blitz record-repair pass, all docs-only: the roadmap's Memory Budget section now states the binding spec §10/§11 envelope (idle 400–500 MB, realistic peak < 600 MB, ~540 MB stress plateau, 600 MB regression bar) instead of the retired 100–300 MB / < 500 MB targets; the theme.rs descriptions in the roadmap, CLAUDE.md, and spec §12.3/§12.4 now describe the `vir_gtk::portal` shim and the crate-tier `base_css` split; the Sepia screenshot task names the real stale file (`main-sepia.png`; `main.png` is current); the Phase 17 Flathub box cites v3.7.3 as the current stable point; and a false 2026-09-05 tag-backfill claim is corrected (only v3.4.1 carries a tag; v3.2.1 and v3.3.1 remain untagged pending the cut-or-record decision).
