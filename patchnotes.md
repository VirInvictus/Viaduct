...
- **The application id is now `io.github.virinvictus.Viaduct`** (decision 64, which v3.8.0 shipped without executing). All fifteen sites move together: the desktop file, metainfo, gschema (id and path), D-Bus service template, Flatpak manifest, meson's `application_id`, the four hicolor icon files, and every code literal. `preferences.rs`'s constant is renamed `SCHEMA_ID` to `APP_ID` and the GTK `application_id`, tray id and icon name, and preferences schema lookups all route through it, so the next identity change is a one-line edit. The Wayland app_id (window class) was verified live on a throwaway profile. **Upgrade note:** the GSettings path moved, so preferences set under the old id are orphaned; carry them over once with `dconf dump /org/virinvictus/Viaduct/ | dconf load /io/github/virinvictus/Viaduct/` (then `dconf reset -r /org/virinvictus/Viaduct/`), and update any Hyprland `windowrulev2` matching the old class.
- **The Flatpak builds offline** (the Flathub blocker no prior audit had seen). The manifest declared `--share=network` and let cargo fetch every crate plus the vir-gtk git dependency at build time; Flathub builds have no network, so the submission could never have built. The Atrium pattern ports over whole: `scripts/flatpak-cargo-generator.py` vendored, `data/cargo-sources.json` generated from `Cargo.lock` (425 crate archives plus the vir-gtk checkout at the locked rev), `CARGO_NET_OFFLINE=true`, and the network build-arg gone. One deliberate divergence from the audit's suggestion: pinning the vir-gtk rev in `Cargo.toml` rewrites the dep URL to the `?rev=` form, which no longer matches the vendored-source replacement and kills the offline build (the first attempt failed exactly that way), so the lockfile plus a new CI freshness guard is the deliberate-bump mechanism, the shape Atrium already ships. Verified end to end: a full `flatpak-builder` download, build, and export completes with no sandbox network.
- **CI hardening:** the container pins `fedora:43` instead of `fedora:latest`, `actions/checkout` and `rust-cache` pin their major tags by SHA, and the job gains a concurrency group and a 30-minute timeout.
- **Screenshot:** `docs/screenshots/main-sepia.png` retaken on the current build (the old capture predated the de-adwaita redesign; its caption described chrome that no longer exists). The metainfo caption matches. Force-light rendering trouble observed while staging light variants is recorded in the Phase 20f hands-on pass, not patched blind.

## v3.9.0: the sync write half connects (2026-09-14)

One data-loss bug that hit every account, the Inoreader write-half lane that turns the remote from a state-reverting trap into a real account, the memory bounds the audits kept recording, and the entire docs backlog the v3.8.0 dependency release deferred. 226 tests, up three; clippy `-D warnings` clean.

- **Fixed:** marking a feed or folder read silently unstarred every starred article in it. The two right-click mark-read paths built status rows with `starred: false` hardcoded, and `upsert_statuses` overwrites every column on conflict. The paths now pull the current statuses and preserve each article's star (never-starred articles still land unstarred); pinned by a regression test next to the upsert op.
- **Inoreader write half (the v3.6.0 read half's missing twin):** locally read or starred articles are now queued as `syncStatus` intent rows, so the next sync sends them to the server instead of the reconcile flipping them right back. Only fields that actually changed queue: a mark-read that preserves the star never sends a `starred=false` edit that could clobber a star another client made after our last sync. Feeds added, removed, renamed, or moved dispatch through the delegate's new server halves (subscribe, unsubscribe, edit-subscription title, and the Reader-API tag quadrants) before the OPML mutates, so the server-authoritative reconcile keeps what the user did. Adoption takes the server's canonical stream id.
- **Inoreader diagnostics and import:** authentication failures surface as a real `NetworkError::Auth` ("no credentials in the keyring", "login rejected with HTTP 401") instead of `RateLimited { retry_after_secs: 0 }` placeholders; the subscription-edit endpoints route through the 429-preserving error mapper so only a genuine 429 arms the sync pause. OPML import runs locally first and uploads best-effort, so an offline import no longer fails outright (residue, recorded in the log: subscriptions the upload never delivered are dropped at the next reconcile, so re-run the import once online). The Activity Log gains account-level sync events: rate-limit pauses with their resume time, quota skips, and sync failures (the log is process-global now, since the delegate predates the window).
- **Fixed:** feed bodies buffer under a hard 10 MB cap streamed through a new shared `read_body_capped` helper, and a mid-body network failure is a fetch error instead of an empty successful response that could persist a partial `max_age`. The same helper bounds favicon discovery (256 KB head scan, 1 MB verify), add-feed discovery, and Reader View, whose 5 MB gate is enforced during the stream instead of after an unbounded read; Reader View also reuses one process-lifetime HTTP client instead of building a fresh one per extraction.
- **Fixed:** timeline fetches take a 1000-row cap (`TIMELINE_FETCH_LIMIT`), so selecting a folder aggregate or an archive-hoarding feed materializes the newest thousand rows instead of every row's full body; the mark-read paths pass an explicit no-limit and still see every article. The refresh cycle pairs feeds with their settings in one bulk worker round-trip (new `FetchMany` op) instead of one round-trip per feed, and 429 cooldowns are process-lifetime now: the refresher builds a fresh fetcher each cycle, so cycle-scoped cooldowns evaporated and a host that asked for backoff was re-hit on the very next cycle (NNW's `DownloadSession` is app-scoped; this matches).
- **Fixed:** favicon discovery backs off 30 minutes between attempts (the `SingleFaviconDownloader` retry-interval port), ending the full home-page GET that dead hosts drew on every refresh cycle forever; the attempt stamp is a new idempotent `favicon_discovery_at` settings column, and success still persists `favicon_url`, ending the probes entirely.
- **Fixed:** the tray spawns on the tokio runtime instead of parking the GTK main loop on ksni's DBus round-trips; the handle returns to the GTK thread through a oneshot, with a stop-pending flag so a flip-off racing the spawn can't leave the tray running.
- **Fixed:** feed-controlled URLs (preferred link, feed link, and the page template's base href) are scheme-validated to http/https and HTML-escaped before substitution into quoted attributes, closing the attribute-breakout that the JS-off settings flag alone was containing; the renderer's module header no longer calls the settings lockdown the "ENTIRE security story".
- **Documented:** the audit backlog that v3.8.0 skipped, in one pass: spec's sidebar/today/reader-view/shortcut/storage/VACUUM/dependency/§12.5/CSP corrections; CLAUDE.md's map rows for the vir-gtk-absorbed files, its section map, title, and storage rows; the roadmap's founding-plan stamp, honest smart-read box, and superseded notes; README's reader-view, vir-gtk, and NewsFlash-gush fixes; ATTRIBUTIONS' libadwaita strike, seven missing dependencies, and the bundled-font OFL attributions; debugging.md's current filter and libadwaita-free prose; and 231 prose em-dashes recast. AppStream releases now carry descriptions (Flathub showed blank notes), starting with the eleven entries whose patchnotes survive and every release from this one on.

## v3.8.0: the vir-gtk widget kit and the gtk4 0.11 platform (2026-09-13)

The dependency-platform waiver deferred at vir-gtk 1.3.0 and the widget-kit first slice land together: viaduct builds on gtk4 0.11 (with webkit6 0.6 matching it) and gets its rows, groups, and alert dialogs from the shared `vir_gtk::widgets` kit; its own `rows.rs` and `alert.rs` are deleted.

- **Changed:** vir-gtk adopted at 1.4.0 (the widget-kit first-slice wave), riding the platform bumps the 1.3.0 waiver deferred: gtk4 0.9 to 0.11 (`v4_16` feature unchanged) and webkit6 0.4 to 0.6 (the matching stack). Viaduct deletes its own `rows.rs` and `alert.rs` entirely: the row builders, `Group`, `Alert`, `Appearance`, and `close_on_escape` now come from `vir_gtk::widgets`, re-exported under the historical `ui::rows` and `ui::close_on_escape` paths so call sites keep resolving. Adaptations: `Alert::new` no longer takes the parent at construction (transient-for derives from the anchor at `present`); `add_response` drops its style argument in favor of `set_response_appearance`; `present(handler)` becomes `connect_response(handler)` + `present(Some(self))` (dismissal now also emits the close response id, which every handler already ignores); the `group()` tuple becomes the `Group` struct (`add`/`widget()` replace the raw list handle); `entry_row` call sites adopt the widened four-`Option` signature. The gtk4 bump lists every implemented interface on `ViaductWindow`'s `glib::wrapper!` (Accessible/Buildable/ConstraintTarget/Native/Root/ShortcutManager) and disambiguates the resulting `focus()` ambiguity to `GtkWindowExt::focus`, per the Atrium precedent. Suite 220 green, clippy `-D warnings` clean.
- **Changed:** vir-gtk adopted at 1.3.0 and 1.2.0 (the consumer waves): the gtk4 0.11 platform port and the StyleManager/StyleScope opt-in APIs; Viaduct's theme shim and its `default_dark = false` light default are unchanged. The 1.3.0 lock ride happened with the 1.4.0 wave above after the gtk4 0.11 bump made the two inseparable; the 1.2.0 wave was lock-only.
...
- **Fixed:** the article pane reserves the scrollbar gutter (NNW `808403b00`, deferred candidate 393). Upstream added `html { scrollbar-gutter: stable; }` to core.css so the leading edge of the article stays put when the scrollbar appears or disappears; ours reflowed every line when a short article followed a long one. The rule rides verbatim in `VIADUCT_PANE_OVERRIDE_CSS`, next to the custom scrollbar it complements: `overflow: auto !important` stays (losing it would re-clip long articles), the 6→10 px currentColor scrollbar is untouched, and a WebKit build without the property drops the declaration harmlessly. A regression test pins both the rule and the overflow invariant; 223 tests pass, up one.
- **Fixed:** the Flatpak builds end to end again (roadmap 491's gate, open since the GNOME 50 bump made the SDK extension the blocker). Four fixes fell out of the first real `flatpak-builder` run. The build sandbox now shares the network during the build (`build-args: --share=network`), because cargo fetches the workspace's crates and the vir-gtk git dependency at build time and the sandbox allowed no DNS at all. The Flatpak now installs the app icon as 256x256 and 512x512 PNGs and skips the SVGs (`-Dsvg-icons=false`; rasters are also what Flathub asks for), because appstreamcli compose and the export stage read icons through GdkPixbuf and the GNOME 50 stacks have no SVG loader in that path at all: any SVG in the payload, even a minimal valid one, aborted the build with "Unrecognized image file format". The manifest drops the `--device=dconf` finish-arg that v3.4.1 added alongside the real `--filesystem=xdg-run/dconf` grant: flatpak itself rejects `dconf` as a device type (`valid types are: dri, all, kvm, shm, input, usb`), so the arg was dead weight at best and a launch-blocking validation error at worst. And a D-Bus activation service file now ships (a data template plus a meson install), because the desktop file's `DBusActivatable=true` hard-requires one at export; from source its Exec points at the installed binary, and the Flatpak overrides it with the `flatpak run` wrapper via `-Ddbus-service-exec`, since the service file is read by the host session bus. From-source and distro installs keep every previous default; the two meson options only add overrides. Verified with a full local `flatpak-builder --install` run: configure, cargo release build, install, appstream compose, export, and installation all complete, and the installed Flatpak was smoke-launched on the live Wayland session (welcome dialog, live feed fetch, themed article render).

## v3.7.4: vir-gtk adoption records and the blitz record repairs (2026-09-06)

No code changes. Six docs-only commits had accumulated on main past the v3.7.3 tag; this release is the record for all of them: the vir-gtk consumer wave's adoption notes, a spec-contract sync that caught the docs up to the shipped app (Inoreader recorded as the shipped remote, the retired memory targets restated as the §10 envelope, the fetcher UA string matched to code), and the 2026-09-06 blitz audit's record repairs.
...
- **Documented:** the blitz record repairs, all docs-only: the roadmap's Memory Budget section now states the binding spec §10/§11 envelope (idle 400–500 MB, realistic peak < 600 MB, ~540 MB stress plateau, 600 MB regression bar) instead of the retired 100–300 MB / < 500 MB targets; the theme.rs descriptions in the roadmap, CLAUDE.md, and spec §12.3/§12.4 describe the `vir_gtk::portal` shim and the crate-tier `base_css` split; the Sepia screenshot task names the real stale file (`main-sepia.png`; `main.png` is current); the Phase 17 Flathub box cites the current stable point; and a false 2026-09-05 tag-backfill claim is corrected (only v3.4.1 had actually been tagged). Decision recorded alongside: v3.2.1 and v3.3.1 stay untagged under the forward-only tag policy, no backfill; this release follows the same policy.
