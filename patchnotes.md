...
- **Timeline polish** (pre4). Three quality-of-life upgrades:
  ...
  - **pre4.2 followup**: dates were missing in smart-feed views (Today / All Unread / Starred) even though the underlying values rendered correctly. The trace showed the date_label was being set fine — but smart-feed timelines aggregate articles across many feeds, producing unusually long titles that pushed the date label off-screen via the row's hbox layout. Fixed by reserving 9 character widths on the date label (`set_width_chars(9)` + `xalign 1.0`), enough to fit the widest relative-date string ("Yesterday") plus padding. Title still ellipsizes via its `EllipsizeMode::End`.
