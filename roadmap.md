...
- [ ] **Refresh while window is closed** *(planned, deferred to Phase 17)*. Architecture written up in `docs/background-service-plan.md`: window-hide-on-close + `xdg-desktop-portal` Background API + D-Bus activation for re-summoning. ~3 focused days of work, decision tree and rejected alternatives documented. NNW's mac equivalent (`NSBackgroundActivityScheduler`) has no Linux analog without a portal client; we already depend on `ashpd` and `viaduct-core/src/network/background.rs` has the request helper.
...
- [ ] Background daemon via `xdg-desktop-portal` Background API for refresh while the UI is closed *(planned — full architecture in `docs/background-service-plan.md`. The `ashpd` request helper already exists at `viaduct-core/src/network/background.rs`; the missing pieces are window-hide-on-close, the `run-in-background` GSetting, the Flatpak manifest permission, and D-Bus activation for re-summoning. ~3 focused days estimated.)*
