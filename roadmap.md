...
- [ ] **In-pane YouTube playback** *(post v1.2.0)*: detect YouTube watch URLs (and other major embeds — Vimeo, PeerTube) in feed body HTML or article links, and play them inside the article pane via a sandboxed `WebKitWebView` instance dedicated to the embed (separate from the article-text WebView so the lockdown profile stays strict for everything else). CSP for the embed view would relax `default-src` to allow YouTube's iframe origin only, with `script-src` still locked off. Alternative: shell out to `mpv` via `gio::AppInfo::launch_default_for_uri` (current Ctrl+Enter behaviour for enclosures) — decide based on user preference + memory cost.
- [ ] **Empty-state flash on sidebar selection** *(v1.2.0 polish followup)*: clicking between feeds briefly shows the "No articles" status page during the timeline rebuild. Cause: `populate_timeline` does `store.remove_all()` (fires items_changed → flips to empty) then `store.append(...)` (fires again → flips back). Fix: replace the two calls with `gio::ListStore::splice(0, n_existing, &new_items)` — one atomic items_changed instead of two. ~5 lines.
