...
* `window.rs` — `ViaductWindow` subclass of `AdwApplicationWindow`. Built via `window.ui` GTK Builder XML. Owns `Arc<LocalAccount>` and `Arc<ImageCache>` (both in `OnceCell` inside the `imp` struct), the `SidebarDataSource`/`SidebarTreeControllerDelegate`/`TreeController`, the `timeline_store`, and `selected_feed_id: RefCell<Option<String>>` (updated by sidebar selection, read by search scope). `wire_models()` loads OPML on startup, wires sidebar-selection → appropriate fetcher (`fetch_articles_by_feed` for feeds, smart-feed dispatch for `Today`/`All Unread`/`Starred`, `fetch_folder_articles` for folders that aggregates children) → timeline store, timeline-selection → `article::render_html`. `wire_search()` binds the sidebar `search_btn` toggle to the `search_bar` reveal state, debounces keystrokes 150ms, resolves the scope at query-fire time (reads `scope_toggle.is_active()` + `selected_feed_id`), and calls `account.search_articles_with_snippets(fts, feed_filter)`. `populate_timeline_with_snippets` builds `ArticleNode::with_snippet` rows so the timeline preview shows the FTS5 excerpt instead of the generic summary.
* `window.ui` — three-pane `AdwNavigationSplitView` scaffolding (sidebar → timeline → article body). Sidebar header bar holds `mark_all_read_btn`, `search_btn` (toggle), `menu_btn`. Timeline pane has a `GtkSearchBar` containing a horizontal box with the `search_entry` (hexpand) and a `scope_toggle` GtkToggleButton ("This feed"). Article pane is a `GtkScrolledWindow` → `GtkTextView` (word-wrap, non-editable, cursor-invisible).
...
* `timeline.rs` — `ArticleNode` (glib wrapper around `Article`, optionally carrying a `snippet: Option<String>` for search-result rows) + `setup_timeline_list_view`. `GtkListView` + `GtkSignalListItemFactory` with **strict widget recycling**. Rendering 10,000 articles must cost the same RAM as rendering 10. Title, feed id, date, 2-line preview. The preview prefers `node.snippet()` when set (FTS5 excerpts) over `article.summary`/`content_text`. Returns `SingleSelection` so the window can drive article rendering from it.
