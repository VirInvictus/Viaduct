...
* `video_thumbs.rs` (v1.3.0) — video-source detection + thumbnail-URL resolution. `VideoSource` enum (`YouTube { id }` / `Vimeo { id }`); `VideoSource::from_url` matches `youtube.com/watch?v=`, `youtu.be/<id>`, `youtube.com/embed/<id>`, `youtube.com/shorts/<id>`, `youtube.com/v/<id>`, `youtube.com/live/<id>`, `youtube-nocookie.com/embed/<id>`, `vimeo.com/<numeric>`, `vimeo.com/video/<numeric>`, `player.vimeo.com/video/<numeric>` (host-prefix-stripped, numeric-ID guard for Vimeo). `detect_video(&Article)` walks `external_url` → `url` → `content_html` / `content_text` / `summary` for the first hit (URL-token extraction is a hand-rolled `extract_url_candidates` scanner — no regex dep). `youtube_thumbnail_url(id)` returns the deterministic `https://i.ytimg.com/vi/<id>/hqdefault.jpg` (480×360, always exists). `thumbnail_url(client, source)` is async — sync return for YouTube, oEmbed lookup for Vimeo via `https://vimeo.com/api/oembed.json?url=…` reading `thumbnail_url` from the JSON. Used by `ui::timeline::spawn_video_thumbnail_fetch` to populate the timeline row's leading `gtk::Picture`.
