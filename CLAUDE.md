...
* `data/` — non-source assets shared across both crates. Holds `org.virinvictus.Viaduct.gschema.xml` (GSettings schema), `themes/` (9 article themes), `fonts/` (Inter + SourceSerif4 + JetBrainsMono + atkinson/), and the build-produced `gschemas.compiled` (gitignored). Future home for icons / appstream metadata once Phase 17 packaging lands. Stays at the repo root; `viaduct/build.rs` walks `CARGO_MANIFEST_DIR.parent()` to find it for `glib-compile-schemas`, and the `include_str!` / `include_bytes!` calls in `viaduct/src/ui/article_renderer.rs` and `viaduct/src/fonts.rs` use three-up `../../../data/...` paths.
