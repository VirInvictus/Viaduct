...
* `preferences_dialog.rs` — `present(parent)` builds an `AdwPreferencesDialog` with a single "General" page: Appearance group with a color-scheme `AdwComboRow` (Follow system / Force light / Force dark; nick ↔ index mapped via `nick_to_index`/`index_to_nick`), Notifications group with a `notifications-on-refresh` `AdwSwitchRow` bound bidirectionally via `gio::Settings::bind`. Color-scheme writes go through `Settings::set_string`; external flips re-sync via `connect_changed`. When `crate::preferences::settings()` returns None (schema not installed in dev env) the dialog renders an inert "Settings unavailable" row instead of crashing.
...
* `timeline.rs` — `ArticleNode` (glib wrapper around `Article`, plus optional `snippet` for search rows and `read`/`starred` exposed as **glib derived properties** so notify-signals drive in-place title restyling without waiting for a row recycle) + `setup_timeline_list_view(list_view, store, FeedNameMap)`. `FeedNameMap` (= `Rc<RefCell<HashMap<feed_id, display_name>>>`) is the resolver passed in by the window; the bind closure reads through it on each row, falling back to the raw `feed_id` when the OPML hasn't loaded yet. `GtkListView` + `GtkSignalListItemFactory` with **strict widget recycling**. Rendering 10,000 articles must cost the same RAM as rendering 10. Title (CSS classes toggled by `apply_read_styling` — `heading` when unread, `dim-label` when read), media indicator (icon + count badge when `article.attachments` is non-empty), date, feed name, 2-line preview. Preview prefers `node.snippet()` (FTS5 excerpt) over `article.summary`/`content_text`. `connect_bind` connects a `notify::read` handler that re-runs `apply_read_styling`; the handler-id is stashed via `unsafe item.set_data` and disconnected in `connect_unbind` so recycled rows don't accumulate handlers across nodes. Returns `SingleSelection` so the window can drive article rendering and keyboard navigation from it.
* `article_renderer.rs` — Phase 6 reading pane. Single neutered `WebKitWebView` instance drives all article rendering. **`apply_locked_down_settings`** disables JS / WebGL / WebRTC / plugins / DevTools / LocalStorage / IndexedDB / app cache / fullscreen / window-open / media-autoplay / back-forward gestures; sets `media_playback_requires_user_gesture(true)`. **`install_link_interceptor`** connects `decide-policy` so every link click (NavigationAction or NewWindowAction with type LinkClicked / FormSubmitted) calls `decision.ignore()` and routes the URL to `gio::AppInfo::launch_default_for_uri` (xdg-open). `Other` / `Reload` / `BackForward` allowed through so the synthetic about:blank load that backs `load_html` works. **`install_hover_url_overlay`** wires `mouse-target-changed` to a `gtk::Label` overlay (`osd` + `caption` style classes, halign=start / valign=end / can-target=False) so hovered link URLs surface in the bottom-left. **`install_image_uri_scheme`** registers `viaduct-img://` on the default `WebContext` (process-wide, idempotent). The handler clones the URISchemeRequest, hops to `glib::spawn_future_local`, then to tokio for `ImageCache::image()`, then back to GTK to call `request.finish()` with a `gio::MemoryInputStream`. Failure path emits `gio::IOErrorEnum::NotFound`. Article HTML bodies run through `sanitize_and_rewrite_image_srcs` first — an `ammonia::Builder` with `viaduct-img` added to `url_schemes` plus an `attribute_filter` that rewrites every `img@src` whose URL starts with `http://` or `https://` to `viaduct-img://i/<percent-encoded-original>`. `percent_encode` / `percent_decode` helpers are inlined (RFC 3986 unreserved set) to avoid pulling in `percent-encoding` as a top-level dep. **Themes**: `THEMES` const array carries all 8 NNW `.nnwtheme` bundles via `include_str!` (Sepia, Appanoose, Biblioteca, Hyperlegible, NewsFax, Promenade, Tiqoe Dark, Verdana Revival); each is a `(template.html, stylesheet.css, dark: bool)` tuple. `select_for_dark_mode` pairs Sepia (light) with Tiqoe Dark (dark); user-facing theme picker queued for v1.2.0 polish. **Macro engine**: `render_with_macros` is a linear-scan port of NNW's `RSCore.MacroProcessor.processMacros()` — `[[key]]` delimiters, unknown keys preserved as literal `[[key]]`. `ArticleSubstitutions` mirrors NNW's `articleSubstitutions()` exactly (title, body, preferred_link, feed_link, byline, datetime_*, date_*, time_*, dateline_style, text_size_class, avatar_src, external_link*) so bundled themes work without modification. **Page wrapper** (`data/themes/page.html`, NNW-shape) carries the strict CSP meta tag — `default-src 'none'; img-src viaduct-img: data:; style-src 'unsafe-inline'; base-uri 'none'; form-action 'none'; frame-ancestors 'none'` — so the WebProcess gets ZERO direct internet access. Two-pass substitution in `render_themed`: inner = theme template + article subs; outer = page wrapper + (style, baseURL, title, inner). Final HTML rides `WebView::load_html(html, base_uri)`.
